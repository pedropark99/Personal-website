...
However, calculating the distance between one point and all points from all other curves
would be impractical. What the Jobard-Lefer algorithm does is calculating the distance
between one point and **all of the points that are near by and that belongs to others (already existing) curves**.

To find the points that are near by, the algorithm uses a second Cartesian grid that is superposed
over the Cartesian plane. In other words, it uses a second Cartesian plane
that uses a different scale, accordingly to the "separating distance" ($d_{sep}$).
@jobard97 call this second Cartesian plane as the "density grid".

So every coordinate in the Cartesian plane can be mapped to a position in the density grid.
