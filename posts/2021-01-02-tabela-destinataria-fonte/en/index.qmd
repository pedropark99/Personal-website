...
However, some confusion can be easily applied in both alternatives. I mean, a student can easily face the following question: "`r emoji::emoji("thinking")` Uhmm... I don't remember very well. Does a *left join* keep the rows in table `A`? Or are the rows in table `B`?"; or else, "`r emoji::emoji("thinking")` Wait! But which of the two tables is on the right side?"
