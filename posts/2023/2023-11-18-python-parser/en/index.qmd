...
def _parse_input(parser_cache: ParserCache) -> ParserCache:
    ...
    if parser_cache.current_token() in ['"', "'"]:
        ...
        parser_cache = _parse_add_operator(parser_cache)
    else:
...
Now, things get a little bit more complicated when we find a plus sign in the middle of the sequence of tokens. Because if we do find a plus sign, that means that this plus sign should be surrounded by operands. In other words, this plus sign connects the left operand to the right operand of this addition operation.

In theory, the previous token in the sequence is in fact the left operand of this addition. The previous token in the sequence is the already parsed token that is present at the top level of the AST, in the parser cache object. That means, that we can get the left operand by executing `parser_cache.ast[-1]`.

But, the big problem here is with the right operand. Because, in theory, the next token in the sequence is the right operand. But that is not really true, or, it would be too easy to be true. If we collect the next token, we get only one single token, and that is the real problem. Because if you think about it, you will probably find examples where the right operand could be composed of a sequence of multiple tokens, and not just a single one token.

The expression `database + '.events'` is an example. Take a look at the tokenizer's output over this expression. The left operand is the first token (`database`), but the right operand, is actually the three last tokens in the sequence (`['"', '.events', '"']`). This means that these three tokens together are the right operand.

```{python}
print(tokenizer('database + ".events"'))
```

As a result, if we advanced the parser with `next_token()`, and run `current_token()` we would get only the token `"`, so the right operand would be incomplete.
...
```{python}
def _parse_add_operator(parser_cache: ParserCache) -> ParserCache:
