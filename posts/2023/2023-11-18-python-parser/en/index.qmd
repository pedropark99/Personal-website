...
You can see at @lst-table that we have a `spark.table()` call to acess the table `blip.events`. This table is stored inside our distributed SQL database. In our current (or "old") infrastructure, every SQL table that we have is concentrated into a single data catalog. 
