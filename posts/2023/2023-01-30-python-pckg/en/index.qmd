...
This is what a build system does. It compiles all archives of your package into a single, compact and shareable format. This means that, when a user downloads your package (through `pip` for example), it downloads this compact and shareable version of your package. There are four main build systems for Python packages on the market, which are `hatchling`, `setuptools`, `flit` and `PDM`.

No matter which one you use, just choose one, anyone. They do the same thing. Most of them use various metadata stored at the `pyproject.toml` file to build your project. The `setuptools` build system is probably the most "famous" system of all. Not because can use two other files to replace