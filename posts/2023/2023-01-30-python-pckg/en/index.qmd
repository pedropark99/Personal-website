...
The `build-system` table is the part of `pyproject.toml` file where you can specify options and configurations for the build system. As an example, to use the `setuptools` build system, you would add these lines to `pyproject.toml`:
...
The `setuptools` build system is more "versatile"^[And I mean these quotation marks, because I am not sure to call a system which can be out of the standard as "being more versatile", or not.] than the others, because it supports other kinds of files, besides `pyproject.toml`.

The `setup.py` file is probably the most "famous", or, the most associable to `setuptools`. In summary, you can replace `pyproject.toml` by the `setup.py` file. According to @myrian2022, this file is supposed to serves two primary purposes:

- It contains various information relevant to your package including options and metadata, such as package name, version, author, license, minimal dependencies, entry points, data files and many more.

- Serves as the command line interface that enables the execution of packaging commands.
