...
No nosso exemplo, as chaves do JOIN são representadas pela coluna `ID`. Logo, o JOIN vai puxar os dados de uma tabela para a outra, utilizando os valores dessa coluna como guia. Podemos visualizar esse processo de pareamento dos dados em @fig-pareamento.
...
Contudo, apesar da tabela `cores_de_pele` não apresentar IDs duplicados, a tabela `alturas` por sua vez, possui três linhas diferentes para o mesmo ID 105. Nesse caso, como o processo de pareamento do JOIN deve se comportar? Bem, o seguinte vai acontecer...

Primeiro, o JOIN vai pegar o ID 105 da tabela `cores_de_pele` e pesquisar por ele ao longo da tabela `alturas`. Como resultado, o JOIN vai localizar três linhas distintas para o ID 105. Segundo, o JOIN percebe que há um desequilíbrio (1 linha de `cores_de_pele` $\times$ 3 linhas de `alturas`). Terceiro, para reequilibrar essa balança, o JOIN vai executar um produto cartesiano entre as linhas dessas duas tabelas.

Ou seja, o JOIN vai retornar como resultado, todas as combinações possíveis entre a linha única de `cores_de_pele` e as 3 linhas de `alturas`. Ou seja, a linha única de `cores_de_pele` é combinada com cada uma das 3 linhas de `alturas`. Como resultado, temos as 3 linhas que vimos na tabela `dados`.
